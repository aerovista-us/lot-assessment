...
export default function PondyLotWorkspace() {
  ...
  return <main className="shell workbench-shell lot-workspace-shell">
    <section className="staff-page-head"><p className="eyebrow">PONDY FLATS · LOT WORKSPACE</p><h1>{lot.label}</h1><p className="lede">{lot.address} · {lot.frontage} access · {lot.informationState.toLowerCase()} information state</p>
      <div className="hero-actions"><Link className="primary-button" href="/workbench/projects/pondy-lot2/candidates">Review candidates</Link><Link className="secondary-button" href="/workbench/solver">Run broad solver</Link></div></section>
    ...
    <section className="lot-workspace-grid">
      <article className="wb-panel"><p className="eyebrow">NEXT ACTION</p><h2>Review intervention-ready options.</h2><p>{interventions.length} candidates retain useful program/site geometry but still have machine-owned blockers. Branch or compare these before inventing a new layout from scratch.</p><Link className="secondary-button" href="/workbench/projects/pondy-lot2/candidates">Open Candidate Library</Link></article>
