...
export function evaluatePlacement(problem: PlacementProblem, candidate: PlacementCandidate): CandidateEvaluation {
  ...
  for (let i = 0; i < candidate.placements.length; i += 1) {
    for (let j = i + 1; j < candidate.placements.length; j += 1) {
      ...
      const integrated = intentionalOverlap(a, b);
      if (!integrated && polygonsIntersect(placementPolygon(a), placementPolygon(b), 0.02)) {
        overlapPass = false;
  ...
  for (const { driveId, result } of sweeps) {
    ...
    for (const pathIssue of result.pathIssues) {
      if (pathIssue.kind === "short-tangent") issues.push(`${driveId}: short tangent ${pathIssue.haveFt}′ < ${pathIssue.needFt}′`);
    }
  }
...
export function shiftPlacement(candidate: PlacementCandidate, id: string, dx: number, dy: number): PlacementCandidate {
  const target = candidate.placements.find((item) => item.id === id);
  const groupId = target?.integrationGroupId;
  return {
    ...candidate,
    placements: candidate.placements.map((item) =>
      item.id === id || (groupId && item.integrationGroupId === groupId)
...
export function shiftDriveControlPoint(candidate: PlacementCandidate, driveId: string, pointIndex: number, dx: number, dy: number): PlacementCandidate {
  return {
    ...candidate,
    drives: candidate.drives.map((drive) => drive.id !== driveId ? drive : {
      ...drive,
      points: drive.points.map((point, index) => index === pointIndex ? [point[0] + dx, point[1] + dy] as Point : point)
    })
  };
}
...
export function boundedRepair(problem: PlacementProblem, source: PlacementCandidate, options?: { stepScheduleFt?: number[]; maxActions?: number; maxStates?: number; }): RepairResult {
  ...
  while (queue.length && tested < maxStates) {
    ...
    for (const drive of state.candidate.drives) {
      ...
      for (const pointIndex of movable) for (const step of steps) for (const [dx, dy] of [[step, 0], [-step, 0], [0, step], [0, -step]] as Point[]) {
        const totalDx = state.actions.filter((a) => a.kind === "shift-drive-point" && a.targetId === drive.id && a.pointIndex === pointIndex).reduce((sum, a) => sum + a.dx, 0) + dx;
        const totalDy = state.actions.filter((a) => a.kind === "shift-drive-point" && a.targetId === drive.id && a.pointIndex === pointIndex).reduce((sum, a) => sum + a.dy, 0) + dy;
        if (Math.hypot(totalDx, totalDy) > limit + 1e-6) continue;
        mutations.push({ candidate: shiftDriveControlPoint(state.candidate, drive.id, pointIndex, dx, dy), action: { kind: "shift-drive-point", targetId: drive.id, pointIndex, dx, dy } });
      }
    }
    ...
    for (const mutation of mutations) {
      ...
      if (evaluation.pass) return { status: "PASS", candidate: mutation.candidate, evaluation, actions, tested };
      if (score(evaluation, actions) < score(best.evaluation, best.actions)) best = { candidate: mutation.candidate, evaluation, actions };
  ...
  return { status: improved ? "NEAR_PASS" : "FAIL", candidate: best.candidate, evaluation: best.evaluation, actions: best.actions, tested };
}
...
export function searchGridPlacements(args: {
  ...
  const recurse = (index: number, placed: AxisAlignedPlacement[]) => {
    ...
    if (index >= args.templates.length) {
      const candidate: PlacementCandidate = { id: `${args.family}-${output.length + 1}`, family: args.family, placements: [...(args.fixedPlacements ?? []), ...placed], drives: args.drives?.map((drive) => ({ ...drive, points: drive.points.map(([x, y]) => [x, y]) })) ?? [] };
      let evaluation = evaluatePlacement(args.problem, candidate);
      let finalCandidate = candidate;
      if (!evaluation.pass && args.repairNearPasses) { const repair = boundedRepair(args.problem, candidate, { maxActions: 2, maxStates: 600 }); finalCandidate = repair.candidate; evaluation = repair.evaluation; }
